...
usage: python3 ivymon.py

prerequisites: py34-pyqt5, ivy for python3 (pip3 install ivy-bus
Copyright (c) 2016-2018 Yannick Jestin <yannick.jestin@gmail.com>
"""
...
class IvyMonView(QtWidgets.QMainWindow):
    ...
    def __init__(self, bus, busaddr, name):
        ...
        self.ui.actionRecord.triggered.connect(lambda:
                                               self.bus.dorecord(self.ui.actionRecord.isChecked()))
        self.ui.actionReplay.triggered.connect(self.bus.doreplay)
        self.ui.actionSave.triggered.connect(self.bus.dosave)
        self.ui.actionLoad.triggered.connect(self.bus.doload)
...
class IvyMon(ivy.IvyServer):
    """une classe pour visualier le trafic ivy sur un bus donne"""
    ...
    def __init__(self, bus):
        ...
        # prepare la memoire pour enregister / rejouer
        self.doi = False
        self.memory = []
        # initialise le bus
    ...
    def sigstop(self, *args):
        ...
        # j'ai recu un SIGSTOP, je quitte le bus
        if self.isAlive:
            self.stop()
        # puis je quitte Qt Main Loop
        QtCore.QCoreApplication.instance().quit()

    def dorecord(self, doi):
        self.doi = doi

    def doreplay(self):
        self.replay = Replay(self.memory, self)

    def doload(self):
        fname = QtWidgets.QFileDialog.getOpenFileName(self.view,
                                                      'Open file', '.',
                                                      'pickle files (*.p)')
        self.memory = pickle.load(open(fname[0], "rb"))

    def dosave(self):
        fname = QtWidgets.QFileDialog.getSaveFileName(self.view,
                                                      'Save file', '.',
                                                      'pickle files (*.p)')
        pickle.dump(self.memory, open(fname[0], "wb"))
...
class Replay():
    """
    a helper class that parses the memory, and sends the messages on the
    Ivy bus with a QTimer based system
    """

    def __init__(self, memory, bus):
        self.memory = memory.copy()
        self.bus = bus
        work = self.memory.copy()
        for i, e in reversed(list(enumerate(self.memory))):
            work[i] = (work[i][0] - work[i-1][0], work[i][1])
        work[0] = (0, work[0][1])
        self.memory = work
        self.index = 0
    ...
    def do_advance(self):
        msg = ''.join(self.memory[self.index][1])
        self.bus.send_msg(msg)
        self.index = self.index + 1
        if self.is_playing:
            if self.index < len(self.memory):
                QtCore.QTimer.singleShot(self.memory[self.index][0],
                                         self.do_advance)
            else:
...
if __name__ == "__main__":
    """
    programme principal
    créer un bus Ivy, puis une vue IvyMon
    """
